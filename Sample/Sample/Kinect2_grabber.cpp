...
pcl::PointCloud<pcl::PointXYZI>::Ptr pcl::Kinect2Grabber::convertRGBDepthToPointXYZI(RGBQUAD* colorBuffer, UINT16* depthBuffer)
{
	...
	for (int y = 0; y < depthHeight; y++){
		for (int x = 0; x < depthWidth; x++, pt++){
			...
			if ((0 <= colorX) && (colorX < colorWidth) && (0 <= colorY) && (colorY < colorHeight)){
				...
				point.intensity = RGBToGreyScale(color.rgbRed, color.rgbGreen, color.rgbBlue); //get greyscale as intensity
				//point.intensity = 0.3*color.rgbRed + 0.3*color.rgbGreen + 0.3*color.rgbBlue;
			}
...
double pcl::Kinect2Grabber::sRBG_to_linear(double x){
	if (x < 0.04045) return x / 12.92;
	return pow((x + 0.055) / 1.055, 2.4);
}
double pcl::Kinect2Grabber::linear_to_sRGB(double y){
